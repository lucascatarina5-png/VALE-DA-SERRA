(function(){
  ...
  function applyCleanup(data,selected){
    ...
    if(chosen.debts.length){debitos=debitos.flatMap(debt=>!debtIds.has(String(debt.id))?[debt]:debt.serverSavedV162?[{...debt,status:'excluido',situacao:'Excluido',situacaoPagamento:'Excluido',excluidoEm:new Date().toISOString()}]:[]);pagamentosDebitos=pagamentosDebitos.filter(item=>!debtIds.has(String(item.debitoId)))}
