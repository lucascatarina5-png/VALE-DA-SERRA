(function(){
  ...
  function paymentMainLocal(pg,p){return String(pg?.localidadePrincipalPagamento||pg?.localidadeCadastroProdutor||p?.local||pg?.localidade||'-').trim()}
  function paymentDeliveryText(pg,p){
    const main=paymentMainLocal(pg,p),locations=Array.isArray(pg?.locaisEntrega)&&pg.locaisEntrega.length?pg.locaisEntrega:(pg?.localidade&&norm(pg.localidade)!==norm(main)?[{local:pg.localidade,litros:pg.litros}]:[{local:main,litros:pg?.litros}]);
    return locations.map(item=>`${item.local}: ${num(item.litros)} L`).join(' • ');
  }
  function paymentHasOutside(pg,p){const main=paymentMainLocal(pg,p),locations=Array.isArray(pg?.locaisEntrega)&&pg.locaisEntrega.length?pg.locaisEntrega:[{local:pg?.localidade||main}];return locations.some(item=>norm(item.local)!==norm(main))}
  ...
  function timelineHtml(d){
    const rows=[];
    (d.milk||[]).forEach(x=>rows.push({at:x.data,icon:'💧',title:`Entrada de ${num(x.qtd)} L — ${turn(x)}`,text:`${x.local||d.producer.local||'Sem localidade'} • ${x.ajusteDe?`ajuste referente a ${date(x.dataReferencia)}`:(x.situacaoPagamento==='Liquidada'?'leite liquidado':'leite ainda pendente')}`}));
  ...
  function milkHtml(d){
    ...
    return `<div class="v135-section-title"><h3>Relatório detalhado de leite</h3>${permitted('entradas')?'<button class="v135-btn success" onclick="v135NewMilk()">＋ Nova entrada</button>':''}</div>
      ...
      <div id="v135MilkTotals"></div><div class="v135-tablewrap"><table class="v135-table"><thead><tr><th>Data</th><th>Turno</th><th>Localidade da entrega</th><th>Litros</th><th>Tipo</th><th>Situação</th><th>Histórico</th>${canEdit?'<th>Ação</th>':''}</tr></thead><tbody id="v135MilkRows"></tbody></table></div>`;
  }
  ...
  window.v135FilterMilk=function(){
    ...
    body.innerHTML=rows.map(x=>{const settled=String(x.situacaoPagamento)==='Liquidada',cor=(x.retificacoes||[]).slice(-1)[0],main=S.statement?.producer?.local||'',entryLocal=x.local||main||'-',cross=!!(main&&entryLocal&&norm(main)!==norm(entryLocal));return `<tr><td><b>${date(x.data)}</b>${x.dataReferencia?`<small>Referente a ${date(x.dataReferencia)}</small>`:''}</td><td>${turn(x)}</td><td><b>${E(entryLocal)}</b>${cross?`<small><span class="v135-pill warn">Fora da principal</span><br>Principal: ${E(main)}</small>`:''}</td><td><b>${num(x.qtd)} L</b></td><td>${x.ajusteDe?'<span class="v135-pill info">Ajuste</span>':'Entrada normal'}</td><td><span class="v135-pill ${settled?'ok':'warn'}">${settled?'LIQUIDADA':'PENDENTE'}</span></td><td>${x.motivo?E(x.motivo):'-'}${cor?`<span class="v135-correction">Retificação: ${num(cor.de)} L → ${num(cor.para)} L<br>${E(cor.motivo||'')}</span>`:''}</td>${canEdit?`<td><button class="v135-btn light" onclick="v135EditMilk('${E(x.id)}')">✏️ ${settled?'Corrigir':'Editar'}</button></td>`:''}</tr>`}).join('')||`<tr><td colspan="${canEdit?8:7}">Nenhuma entrada encontrada neste filtro.</td></tr>`;
  };
  ...
  window.v135PrintProducer=function(){
    ...
    w.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Ficha - ${E(p.nome)}</title><style>@page{size:A4;margin:13mm}*{box-sizing:border-box}body{font:12px Arial;color:#183553;margin:0}h1,h2{color:#092f60;margin:0 0 8px}.head{border-bottom:3px solid #0d4d88;padding-bottom:12px;margin-bottom:14px}.meta{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-bottom:14px}.box{border:1px solid #cfdbe8;border-radius:8px;padding:9px}.box b{display:block;font-size:15px;margin-top:3px}.carry{padding:9px;border-radius:8px;background:#fff1e5;color:#8b3e00;font-weight:800;margin:-5px 0 14px}table{width:100%;border-collapse:collapse;margin:7px 0 16px}th{background:#eaf1f8;text-align:left;padding:6px}td{border-bottom:1px solid #dce5ee;padding:6px;vertical-align:top}.total{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-bottom:14px}.foot{text-align:center;color:#677789;border-top:1px solid #ccd6e0;padding-top:9px;margin-top:18px}@media print{button{display:none}}</style></head><body><div class="head"><h1>VALE DA SERRA LATICÍNIOS</h1><div>Ficha completa do produtor • emitida em ${new Date().toLocaleString('pt-BR')}</div></div><h2>${E(p.nome)}</h2><div class="meta"><div class="box">Código<b>${E(p.codigo||'-')}</b></div><div class="box">Localidade principal<b>${E(p.local||'-')}</b></div><div class="box">WhatsApp<b>${E(p.whatsapp||'-')}</b></div><div class="box">Tanqueiro<b>${E(p.tanqueiro||'-')}</b></div><div class="box">Caminhão<b>${E(p.caminhao||'-')}</b></div><div class="box">Status<b>${E(statusProd(p.id))}</b></div></div><h2>${periodLabel(N(q.numero))} de ${E(monthName(q.mes))}</h2><div class="total"><div class="box">Litros<b>${num(q.litros)} L</b></div><div class="box">Bruto<b>${money(f.gross)}</b></div><div class="box">Descontos incluídos<b>${money(f.eligible)}</b></div><div class="box">Líquido<b>${money(f.net)}</b></div></div>${f.carry?`<div class="carry">Saldo devedor estimado que continuará em aberto: ${money(f.carry)}</div>`:''}${printTable('Entradas de leite',['Data','Turno','Localidade da entrega','Litros','Situação'],(d.milk||[]).map(x=>[date(x.data),turn(x),x.local||p.local||'-',num(x.qtd)+' L',x.situacaoPagamento]))}${printTable('Pedidos do Galpão',['Data','Pedido','Produtos','Status','Total'],orders.map(x=>[date(x.created_at),'#'+String(x.id).slice(0,8).toUpperCase(),(x.items||[]).map(i=>`${i.product_name} — ${num(i.quantity)} ${i.unit||'un'}`).join('; '),orderStatus(x.status),money(x.total)]))}${printTable('Retiradas do Galpão',['Data','Produto','Quantidade','Valor'],moves.map(x=>[date(x.business_date||x.created_at),x.product_name,num(x.quantity)+' '+(x.unit||'un'),money(N(x.quantity)*N(x.unit_price))]))}${printTable('Compras na Loja / PDV',['Data','Produtos','Pagamento','Total'],sales.map(x=>[date(x.business_date||x.created_at),(x.items||[]).map(i=>`${i.product_name} — ${num(i.quantity)}`).join('; '),paymentName(x.payment_method),money(x.total)]))}${printTable('Débitos',['Data','Descrição','Valor','Saldo'],ds.map(x=>[date(x.data),x.descricao,money(x.valor),money(debitBalance(x))]))}${printTable('Pagamentos do leite',['Período','Localidade da entrega','Litros','Débitos aplicados','Saldo restante','Líquido','Data'],ps.map(x=>[`${monthName(x.mes)} • ${x.quinzena}ª quinzena`,x.localidade||p.local||'-',num(x.litros)+' L',money(x.totalDebitos),money(x.saldoDebitosApos),money(x.valorPago),date(x.dataPagamento)]))}<div class="foot">Vale da Serra — ficha interna consolidada do produtor</div><script>window.onload=()=>setTimeout(()=>window.print(),180)<\/script></body></html>`);w.document.close();
  };
