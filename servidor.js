...
app.post('/api/audit/event',auth,async(req,res)=>{
  try {
    ...
    const allowed=new Set([
      ...
      'LEITE_ENTRADA_REGISTRADA','LEITE_ENTRADA_EDITADA','LEITE_ENTRADA_CORRIGIDA','LEITE_ENTRADA_EXCLUIDA',
...
app.get('/api/producers/:id/statement',auth,async(req,res)=>{
  try{
    ...
    const orders=await pool.query(`SELECT o.*,
      ...
      FROM app_inventory_orders o LEFT JOIN app_inventory_order_items i ON i.order_id=o.id
      WHERE o.producer_id=$1 OR lower(trim(COALESCE(o.producer_name,'')))=lower(trim($2))
      GROUP BY o.id ORDER BY o.created_at DESC LIMIT 1000`,[producerId,producerName]);
...
async function v168CashAnswer(req){
  ...
  return {intent:'cash',answer:sessions.rowCount===1?`O caixa está aberto. O valor esperado em dinheiro na gaveta é ${v168Money(expectedTotal)}.`:`Existem ${sessions.rowCount} caixas abertos, com ${v168Money(expectedTotal)} esperados em dinheiro.`,calculation:'Valor esperado em dinheiro = abertura do caixa + vendas em dinheiro + suprimentos − sangrias. Pix, cartão, boleto e desconto no leite aparecem no relatório, mas não entram no dinheiro da gaveta.',details};
}
...
async function v169StockAnswer(req,question,context={}){
  ...
  if(matched){
    const sameName=products.filter(x=>v168Norm(x.name)===v168Norm(matched.name));
    if(sameName.length>1&&!wantsStore&&!wantsWarehouse)return {intent:'stock-clarification',entity:matched.name,needs_clarification:true,answer:`Encontrei ${matched.name} na Loja e no Galpão. Sobre qual estoque você quer saber?`,details:sameName.map(detail)};
...
// V170 - Assistente administrativa. Amplia interpretação sem permitir que a
// camada de linguagem invente números: todo valor continua calculado no servidor.
function v170Distance(a,b){a=v168Norm(a);b=v168Norm(b);const row=Array.from({length:b.length+1},(_,i)=>i);for(let i=1;i<=a.length;i++){let previous=row[0];row[0]=i;for(let j=1;j<=b.length;j++){const saved=row[j],cost=a[i-1]===b[j-1]?0:1;row[j]=Math.min(row[j]+1,row[j-1]+1,previous+cost);previous=saved}}return row[b.length]}
function v170ProducerScore(question,producer){
  const q=v168Norm(question),names=[producer.nome,producer.name,producer.apelido,producer.nickname].filter(Boolean).map(v168Norm);let best=0;
  for(const name of names){if(name.length<2)continue;if(q.includes(name))best=Math.max(best,120+name.length);const parts=name.split(' ').filter(x=>x.length>2);for(const part of parts)if(q.split(' ').some(word=>word===part||word.startsWith(part)||part.startsWith(word)))best=Math.max(best,25+part.length);const words=q.split(' ').filter(x=>x.length>2);for(const word of words){const distance=v170Distance(word,name);if(distance<=2)best=Math.max(best,18-distance)}}return best;
}
function v170DebitBalance(state,debt){
  const id=String(debt.id||''),original=v158DebitValue(debt),ledger=Array.isArray(state.pagamentosDebitos)?state.pagamentosDebitos:[],payments=Array.isArray(state.pagamentos)?state.pagamentos:[];let paid=ledger.filter(x=>String(v159DebtRef(x))===id).reduce((sum,x)=>sum+Number(x.valor??x.amount??0),0);
  payments.forEach(payment=>{const applications=Array.isArray(payment.debitApplications)?payment.debitApplications:[],application=applications.find(x=>String(v159DebtRef(x))===id),hasLedger=ledger.some(x=>String(v159DebtRef(x))===id&&String(x.pagamentoId??x.paymentId??'')===String(payment.id??''));if(application&&!hasLedger)paid+=Number(application.amount??application.amountApplied??application.valor??(Number(application.balanceBefore||0)-Number(application.balanceAfter||0)))||0;else if(!applications.length&&!hasLedger&&Array.isArray(payment.debitIds)&&payment.debitIds.some(value=>String(value)===id))paid+=original});return Math.max(0,original-paid);
}
async function v170ProducerAnswer(req,question){
  if(!v168Allowed(req.user,'pesquisa')&&!v168Allowed(req.user,'produtores')&&!v168Allowed(req.user,'relatorios'))return null;
  const result=await pool.query("SELECT data FROM app_state WHERE id='vale-da-serra'"),state=result.rows[0]?.data||{},producers=Array.isArray(state.produtores)?state.produtores:[],ranked=producers.map(producer=>({producer,score:v170ProducerScore(question,producer)})).filter(x=>x.score>=25).sort((a,b)=>b.score-a.score);
  if(!ranked.length)return null;if(ranked.length>1&&ranked[0].score===ranked[1].score)return {intent:'producer-ambiguous',answer:'Encontrei mais de um produtor parecido. Escolha um dos nomes mostrados na tela.',details:ranked.slice(0,8).map(x=>({title:x.producer.nome||x.producer.name||'Produtor',subtitle:x.producer.local||x.producer.localidade||'Sem localidade'}))};
  const producer=ranked[0].producer,id=String(producer.id),entries=(Array.isArray(state.lancamentos)?state.lancamentos:[]).filter(x=>String(x.prodId??x.producer_id??'')===id).sort((a,b)=>String(b.data||'').localeCompare(String(a.data||''))),todayResult=await pool.query("SELECT (NOW() AT TIME ZONE 'America/Sao_Paulo')::date::text today,to_char(NOW() AT TIME ZONE 'America/Sao_Paulo','YYYY-MM') month"),today=todayResult.rows[0].today,month=todayResult.rows[0].month,day=Number(today.slice(8,10)),monthParts=month.split('-').map(Number),lastDay=new Date(monthParts[0],monthParts[1],0).getDate(),start=month+(day<=15?'-01':'-16'),end=month+(day<=15?'-15':'-'+String(lastDay).padStart(2,'0'));
  const litersOf=list=>list.reduce((sum,x)=>sum+Number(x.qtd??x.liters??0),0),todayEntries=entries.filter(x=>String(x.data||'').slice(0,10)===today),fortnight=entries.filter(x=>String(x.data||'')>=start&&String(x.data||'')<=end),monthEntries=entries.filter(x=>String(x.data||'').startsWith(month));
  const dates=[];for(let offset=0;offset<10;offset++){const date=new Date(today+'T12:00:00-03:00');date.setDate(date.getDate()-offset);dates.push(date.toISOString().slice(0,10))}const average=dates.reduce((sum,date)=>sum+litersOf(entries.filter(x=>String(x.data||'').slice(0,10)===date)),0)/10;
  const debts=(Array.isArray(state.debitos)?state.debitos:[]).filter(x=>!v159DebtExcluded(x)&&String(v158ProducerRef(x))===id),pendingDebts=debts.map(x=>({...x,balance:v170DebitBalance(state,x)})).filter(x=>x.balance>0.005),debtTotal=pendingDebts.reduce((sum,x)=>sum+x.balance,0),fortnightLiters=litersOf(fortnight),gross=fortnightLiters*2.30,net=Math.max(0,gross-debtTotal),last=entries[0],q=v168Norm(question);
  let answer=`${producer.nome||producer.name}: hoje possui ${v168Number(litersOf(todayEntries))} litros registrados. Nesta quinzena são ${v168Number(fortnightLiters)} litros e neste mês ${v168Number(litersOf(monthEntries))} litros.`;
  if(q.includes('receber')||q.includes('liquido')||q.includes('pagamento')||q.includes('debito'))answer+=` O valor bruto estimado da quinzena é ${v168Money(gross)}, existem ${v168Money(debtTotal)} em débitos pendentes e o líquido estimado é ${v168Money(net)}.`;
  if(q.includes('media')||q.includes('caiu')||q.includes('diminu'))answer+=` A média dos últimos dez dias corridos é ${v168Number(average)} litros por dia.`;
  if(q.includes('ultima')||q.includes('ultimo'))answer+=last?` A última entrada foi de ${v168Number(last.qtd??last.liters)} litros em ${String(last.data||'').slice(0,10)}.`:' Não existe entrada registrada para esse produtor.';
  return {intent:'producer',entity:producer.nome||producer.name,answer,calculation:`Quinzena atual: ${start} até ${end}. Bruto: ${v168Number(fortnightLiters)} litros × R$ 2,30. Líquido: bruto menos débitos ainda não pagos. A média considera dez dias corridos, inclusive dias sem entrega.`,details:[{title:`Hoje: ${v168Number(litersOf(todayEntries))} litros`,subtitle:`Quinzena: ${v168Number(fortnightLiters)} • Mês: ${v168Number(litersOf(monthEntries))}`},{title:`Estimativa líquida: ${v168Money(net)}`,subtitle:`Bruto ${v168Money(gross)} • Débitos ${v168Money(debtTotal)}`},{title:`Média de 10 dias: ${v168Number(average)} litros/dia`,subtitle:last?`Última entrada: ${String(last.data||'').slice(0,10)} • ${v168Number(last.qtd??last.liters)} litros`:'Sem entrada registrada'}],action:{type:'navigate',target:'pesquisa',mobileTarget:'produtores',label:'Abrir ficha do produtor'}};
}
async function v170CompanySummary(req){
  const details=[],parts=[];
  if(v168Allowed(req.user,'relatorios')){const r=await pool.query("SELECT data FROM app_state WHERE id='vale-da-serra'"),state=r.rows[0]?.data||{},today=(await pool.query("SELECT (NOW() AT TIME ZONE 'America/Sao_Paulo')::date::text today")).rows[0].today,entries=(Array.isArray(state.lancamentos)?state.lancamentos:[]).filter(x=>String(x.data||'').slice(0,10)===today),liters=entries.reduce((sum,x)=>sum+Number(x.qtd??x.liters??0),0);parts.push(`${v168Number(liters)} litros de leite`);details.push({title:`Leite hoje: ${v168Number(liters)} litros`,subtitle:`${entries.length} entrada(s) registrada(s)`})}
  if(v168Allowed(req.user,'loja')){const params=[],filter=isAdminUser(req.user)?'':` AND username=$1`;if(!isAdminUser(req.user))params.push(String(req.user.username||''));const sales=await pool.query(`SELECT COUNT(*)::int count,COALESCE(SUM(total),0) total FROM app_store_sales WHERE status<>'cancelada' AND (created_at AT TIME ZONE 'America/Sao_Paulo')::date=(NOW() AT TIME ZONE 'America/Sao_Paulo')::date${filter}`,params);parts.push(`${sales.rows[0].count} vendas e ${v168Money(sales.rows[0].total)}`);details.push({title:`Vendas hoje: ${v168Money(sales.rows[0].total)}`,subtitle:`${sales.rows[0].count} operação(ões)`})}
  if(v168Allowed(req.user,'estoque')){const low=await pool.query(`SELECT COUNT(*)::int count FROM app_inventory_products p LEFT JOIN LATERAL(SELECT COALESCE(SUM(CASE WHEN type='entrada' THEN quantity ELSE -quantity END),0) balance FROM app_inventory_movements WHERE product_id=p.id)m ON TRUE WHERE COALESCE(p.active,TRUE)=TRUE AND COALESCE(m.balance,0)<=p.min_stock`),orders=await pool.query(`SELECT COUNT(*)::int count FROM app_inventory_orders WHERE status IN('pendente','separado','parcial')`);parts.push(`${low.rows[0].count} produtos do Galpão em atenção`);details.push({title:`Galpão: ${low.rows[0].count} estoque(s) baixo(s)`,subtitle:`${orders.rows[0].count} pedido(s) aguardando conclusão`})}
  return {intent:'company-summary',answer:parts.length?`Resumo de hoje: ${parts.join('; ')}.`:'Seu usuário não possui módulos disponíveis para montar o resumo.',details,action:{type:'navigate',target:'painel',mobileTarget:'home',label:'Abrir Painel'}};
}
...
async function v170AnomalyAnswer(req){
  if(!v168Allowed(req.user,'relatorios'))throw Object.assign(new Error('Seu usuário não possui permissão para consultar análises.'),{statusCode:403});
  const r=await pool.query("SELECT data FROM app_state WHERE id='vale-da-serra'"),state=r.rows[0]?.data||{},producers=Array.isArray(state.produtores)?state.produtores:[],entries=Array.isArray(state.lancamentos)?state.lancamentos:[],today=(await pool.query("SELECT (NOW() AT TIME ZONE 'America/Sao_Paulo')::date::text today")).rows[0].today,alerts=[];
  for(const producer of producers){const own=entries.filter(x=>String(x.prodId??x.producer_id??'')===String(producer.id)),todayLiters=own.filter(x=>String(x.data||'').slice(0,10)===today).reduce((s,x)=>s+Number(x.qtd??x.liters??0),0);let sum=0;for(let offset=1;offset<=10;offset++){const date=new Date(today+'T12:00:00-03:00');date.setDate(date.getDate()-offset);const day=date.toISOString().slice(0,10);sum+=own.filter(x=>String(x.data||'').slice(0,10)===day).reduce((s,x)=>s+Number(x.qtd??x.liters??0),0)}const average=sum/10;if(average>0&&todayLiters>0){const variation=(todayLiters-average)/average*100;if(Math.abs(variation)>=30)alerts.push({producer,variation,todayLiters,average})}}
  const duplicateMap=new Map();for(const entry of entries){const key=[entry.prodId??entry.producer_id,entry.data,entry.turno??entry.period??'',Number(entry.qtd??entry.liters??0)].join('|');duplicateMap.set(key,(duplicateMap.get(key)||0)+1)}const duplicates=[...duplicateMap.values()].filter(x=>x>1).length;
  const cash=v168Allowed(req.user,'loja')?await pool.query(`SELECT opened_by,difference,closed_at FROM app_store_cash_sessions WHERE status='fechado' AND ABS(COALESCE(difference,0))>0.01 ORDER BY closed_at DESC LIMIT 10`):{rows:[]};
  const details=alerts.sort((a,b)=>Math.abs(b.variation)-Math.abs(a.variation)).slice(0,30).map(x=>({title:`${x.producer.nome||x.producer.name}: ${x.variation>0?'+':''}${v168Number(x.variation,1)}%`,subtitle:`Hoje ${v168Number(x.todayLiters)} litros • Média anterior ${v168Number(x.average)} litros`}));cash.rows.forEach(x=>details.push({title:`Diferença de caixa: ${v168Money(x.difference)}`,subtitle:`Operador ${x.opened_by||'-'} • ${new Date(x.closed_at).toLocaleString('pt-BR')}`}));if(duplicates)details.push({title:`${duplicates} possível(is) duplicidade(s) de leite`,subtitle:'Mesmo produtor, data, turno e quantidade; confira antes de excluir qualquer registro.'});
...
function v171RewriteQuestion(question,context){
  ...
  if(!/^(nao|não|quis dizer|estou falando|do |da )/.test(String(question||'').trim().toLocaleLowerCase('pt-BR')))return question;
  if(['stock','stock-clarification','stock-forecast'].includes(lastIntent)){
    if(q.includes('galpao')||q.includes('deposito'))return `estoque do galpão${entity?' de '+entity:''}`;
    if(q.includes('loja')||q.includes('pdv'))return `estoque da loja${entity?' de '+entity:''}`;
  }
  if(lastIntent==='sales'||lastIntent==='sales-comparison'){
    if(q.includes('ontem'))return 'vendas de ontem comparadas com hoje';
    if(q.includes('pix'))return 'vendas de hoje no Pix';
    if(q.includes('dinheiro'))return 'vendas de hoje em dinheiro';
  }
  return question;
}
function v171SocialAnswer(req,question,context){
...
app.post('/api/mascot/query',auth,async(req,res)=>{try{
  ...
  else if(!result&&(q.includes('fora do normal')||q.includes('anomalia')||q.includes('duplicad')||q.includes('diminuiu')||q.includes('caiu a producao')||q.includes('diferenca de caixa')))result=await v170AnomalyAnswer(req);
  ...
  if(!result&&(q.includes('leite')||q.includes('produtor')||q.includes('entreg')))result=await v168MilkAnswer(req,question);
  if(!result)result={intent:'help',answer:'Posso pesquisar produtores mesmo com nome incompleto, resumir a empresa, prever a duração do estoque, encontrar situações fora do normal, explicar cálculos, preparar relatórios, consultar vendas, caixa, débitos, leite, tanques e abrir telas.',details:[{title:'Experimente perguntar',subtitle:'Como está a empresa hoje? • Quanto José receberá? • Para quantos dias dá o milho? • O que está fora do normal? • Prepare o relatório das vendas de hoje.'}]};
  if(wantsReport)result=v170AsReport(result);
  await audit(req.user,'MASCOTE_CONSULTA',{intent:result.intent,question:originalQuestion,understood:question});res.json({ok:true,question:originalQuestion,understood_question:question,user:{name:req.user.name,role:req.user.role},...result,generated_at:new Date().toISOString()});
