(function(){
'use strict';
if(window.ValeMascotAssistant?.version>=172)return;
...
let memory=[];try{memory=JSON.parse(localStorage.getItem('vds_v172_conversation')||localStorage.getItem('vds_v171_conversation')||'[]');if(!Array.isArray(memory))memory=[]}catch(_){memory=[]}
...
function ensurePanel(){
 ...
 panel=document.createElement('section');panel.id='v169Assistant';panel.setAttribute('role','dialog');panel.setAttribute('aria-label','Conversa com a vaquinha inteligente');panel.innerHTML='<header><span>🐮</span><b>Vaquinha • Entendimento Inteligente V172</b><button id="v169AssistantClose" aria-label="Fechar">×</button></header><div id="v169AssistantBody"></div>';document.body.appendChild(panel);document.getElementById('v169AssistantClose').onclick=()=>panel.classList.remove('open');return panel;
}
...
function show(question,result,loading=false){
 ...
 const understood=result.understood_question&&String(result.understood_question)!==String(question)?'<div class="v170-calculation"><b>Entendi como:</b> '+E(result.understood_question)+'</div>':'';
 body.innerHTML=loading?'<div class="v169-loading">Entendendo sua pergunta e consultando os dados...</div>':'<div class="v169-q">'+(question?'Você: '+E(question):'Aviso automático')+'</div>'+understood+'<div class="v169-a">'+E(result.answer||'')+'</div><div class="v169-meta">Dados consultados às '+E(time(result.generated_at||new Date()))+'</div>'+((result.details||[]).map(row=>'<div class="v169-row"><b>'+E(row.title||'')+'</b><small>'+E(row.subtitle||row.message||'')+'</small></div>').join(''))+(result.calculation?'<div class="v170-calculation"><b>Como calculei:</b><br>'+E(result.calculation)+'</div>':'')+actionHtml(result.action);
...
async function ask(question,speechAlternatives=[],speechConfidences=[]){
 question=String(question||'').trim();if(!question)return;show(question,{},true);
 try{const result=await request('/api/mascot/query',{method:'POST',body:JSON.stringify({question,speech_alternatives:speechAlternatives,speech_confidences:speechConfidences,context:{...lastContext,history:memory.slice(-8)}})});if(result.clear_context){memory=[];lastContext=null}else{lastContext={intent:result.intent||'',entity:result.entity||'',question,understood:result.understood_question||question};memory.push(lastContext);memory=memory.slice(-8)}try{localStorage.setItem('vds_v172_conversation',JSON.stringify(memory))}catch(_){}show(question,result);deliver(result);if(['navigate','safe-draft'].includes(result.intent))setTimeout(()=>navigate(result.action),450);return result}
...
function askRecognized(candidates,confidences){const list=Array.isArray(candidates)?candidates.filter(Boolean):[];return ask(list[0]||'',list.slice(1),Array.isArray(confidences)?confidences:[])}
function clearConversation(){memory=[];lastContext=null;try{localStorage.removeItem('vds_v172_conversation');localStorage.removeItem('vds_v171_conversation')}catch(_){}return true}
async function checkAlerts(force=false){
 if(!token())return null;const key='vds_v172_alert_check',last=Number(localStorage.getItem(key)||0);if(!force&&Date.now()-last<30*60*1000)return null;
...
window.ValeMascotAssistant=Object.freeze({version:172,ask,askRecognized,clearConversation,checkAlerts,printResult,open:ensurePanel,close:()=>panel?.classList.remove('open'),navigate});
document.dispatchEvent(new CustomEvent('vds:mascot-assistant-ready',{detail:{version:172}}));setTimeout(()=>checkAlerts(false),5500);
})();
