(function(){
  ...
  function enrichReport(raw){
    const rows=(raw.rows||[]).map(row=>{
      ...
      return {...row,debts,applied,bank,bankText:bankText(row.producer),paymentLabel:paymentLabel(row.producer?.formaPagamento)};
    });
    ...
    return {...raw,rows,sources,bankSummary};
  }
