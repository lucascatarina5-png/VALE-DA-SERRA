(function(){
'use strict';
if(window.ValeMascotAssistant?.version>=171)return;
...
let memory=[];try{memory=JSON.parse(localStorage.getItem('vds_v171_conversation')||'[]');if(!Array.isArray(memory))memory=[]}catch(_){memory=[]}
...
function ensurePanel(){
 ...
 panel=document.createElement('section');panel.id='v169Assistant';panel.setAttribute('role','dialog');panel.setAttribute('aria-label','Conversa com a vaquinha inteligente');panel.innerHTML='<header><span>🐮</span><b>Vaquinha • Conversa Natural V171</b><button id="v169AssistantClose" aria-label="Fechar">×</button></header><div id="v169AssistantBody"></div>';document.body.appendChild(panel);document.getElementById('v169AssistantClose').onclick=()=>panel.classList.remove('open');return panel;
}
...
async function ask(question){
 ...
 try{const result=await request('/api/mascot/query',{method:'POST',body:JSON.stringify({question,context:{...lastContext,history:memory.slice(-8)}})});lastContext={intent:result.intent||'',entity:result.entity||'',question,understood:result.understood_question||question};memory.push(lastContext);memory=memory.slice(-8);try{localStorage.setItem('vds_v171_conversation',JSON.stringify(memory))}catch(_){}show(question,result);deliver(result);if(['navigate','safe-draft'].includes(result.intent))setTimeout(()=>navigate(result.action),450);return result}
...
async function checkAlerts(force=false){
 if(!token())return null;const key='vds_v171_alert_check',last=Number(localStorage.getItem(key)||0);if(!force&&Date.now()-last<30*60*1000)return null;
...
window.ValeMascotAssistant=Object.freeze({version:171,ask,checkAlerts,printResult,open:ensurePanel,close:()=>panel?.classList.remove('open'),navigate});
document.dispatchEvent(new CustomEvent('vds:mascot-assistant-ready',{detail:{version:171}}));setTimeout(()=>checkAlerts(false),5500);
})();
